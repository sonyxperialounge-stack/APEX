...
## 2026-09-11 — WP-087 DONE (Doctor completion, 45 §4 registry)
- src/engines/doctor.ts: every 45 §4 DOC-* id now implemented across 11 areas (RUN, PROJ, HOME, LOCK, MIG, MEM, ARC, SKL, EXT, CAP, PKG). Seven sanctioned repairs on derived state only (home-ensure, home-tempfiles, mig-journal, lock-stale, index-rebuild, hot-view, payload-sync). DOC-PKG-SYNC runs the real sync-payload.mjs --check (35 managed files byte-compared); repair re-syncs then re-verifies. DOC-CAP-DISCOVERY counts exposure through the real mayExpose gate — available / policy-blocked / unavailable are distinct counts. DOC-MIG-JOURNAL-PROJ scans the project .apex/migrations/journal.json (WP-083 wiring gap closed; future project schemaVersion > 2 → BLOCKED, read-only).
- Bugs fixed while closing: (1) CAP block unreachable for {areas:["CAP"]} — outer home-branch gate lacked want("CAP") (DOC-T06 caught it); (2) "telemetry" wording tripped the X-004 no-analytics sourcescan — reworded to "outcome records", invariant stays absolute; (3) DOC-LOCK-01 was not implemented — merged DOC-HOME-LOCKS split into DOC-LOCK-01 (writability via resolved mode, never a probe write) / DOC-LOCK-02 (stale) / DOC-LOCK-03 (live, informational).
- Test fixes: DOC-T02 seeded an illegal archive event type ("note") — now "decision"; DOC-T05 called addRequirement before ledger.init(), and init() overwrites REQUIREMENTS.md with an empty list — reordered init() first.
