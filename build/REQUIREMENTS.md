...
| MCP-004 | Tools: `apex_init`, `apex_status`, `apex_req_add`, `apex_req_list`, `apex_req_status`, `apex_verify`, `apex_snapshot`, `apex_rollback`, `apex_delegate`, `apex_subagent_status`, `apex_council`, `apex_gate`, `apex_handoff`, `apex_memory_read`, `apex_memory_write`, `apex_findings`, `apex_task_result` (MCP-010 polling), `apex_models` (FLT-004 pre-dispatch check), plus `apex_check`, `apex_resume`, `apex_decision`, `apex_finding` as documented additions | All listed, all callable, all schema-valid; the test asserts THIS list verbatim | per-tool test |
